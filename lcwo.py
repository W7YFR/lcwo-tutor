...
# The report's stylesheet and application code live next to the extension that
# also uses them: one implementation, inlined here and loaded as files there.
ASSET_DIR = Path(os.environ.get("LCWO_ASSETS", APP_DIR / "lcwo-tools" / "src" / "report"))
...
def build_report(views: list[GroupView], title: str = "LCWO progress",
    ...
    return (HTML_SHELL
            ...
            .replace("__CSS__", asset("report.css"))
            .replace("__APP__", asset("app.js"))
            .replace("__DATA__", payload))


def asset(name: str) -> str:
    """Read a report asset, shared with the browser extension."""
    path = ASSET_DIR / name
    try:
        return path.read_text(encoding="utf-8")
    except OSError as exc:
        raise SystemExit(f"cannot read the report asset {path}: {exc}\n"
                         "  set LCWO_ASSETS if it lives somewhere else")
